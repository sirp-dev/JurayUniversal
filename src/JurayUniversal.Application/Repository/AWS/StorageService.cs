...
namespace JurayUniversal.Application.Services.AWS
{
    public class StorageService : IStorageService
    {
        ...
        public StorageService(IConfiguration config/*, IAmazonS3 client*/, Persistence.EF.SQL.DashboardDbContext context)
        {
            _config = config;
            _context = context;
            //_client = client;
        ...
        public async Task<S3ResponseDto> UploadFileAsync(Application.Dtos.AwsDtos.S3Object obj, AwsCredentials awsCredentialsValues)
        {
        ...
        public async Task<FileReturnResponseDto> UploadFileReturnUrlAsync(JurayUniversal.Application.Dtos.AwsDtos.S3Object obj, AwsCredentials awsCredentialsValues, string old_key)
        {
            var credentials = new BasicAWSCredentials(awsCredentialsValues.AccessKey, awsCredentialsValues.SecretKey);
        ...
        {
            var supw = _context.SuperSettings.FirstOrDefault();
            if (supw != null)
            {
                if (!String.IsNullOrEmpty(supw.BucketName))
                {
                    return Task.FromResult(supw.BucketName);
                }
            }
